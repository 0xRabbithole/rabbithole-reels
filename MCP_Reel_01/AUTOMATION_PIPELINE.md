...
## Cover image (grid-optimized) — repeatable

Use `reel_cover_template.html`. Every Reel needs a custom cover, because **Instagram crops the cover to a center 1:1 square in your profile grid** — anything outside that square gets chopped.

Per video, swap only:
- The kicker (e.g. "AI · explained")
- The question / hook (e.g. "What is an **MCP**?")
- The one-line sub
Everything else (brand, blue highlight, logo, the center-square safe zone) stays.

Rule: **all text must sit inside the center square.** Toggle the "grid guide" button to check, then screenshot the stage at 1080×1920 and upload it as the custom cover.

---

## Distribution — ManyChat + Buffer (repeatable, many/day)

Neither has an MCP connector, so both are **browser-driven** (Claude clicks them; you stay logged in).

**ManyChat (comment → DM) — the scalable pattern**
- Account: Rabbithole Consulting · channel: Instagram **@rabbitholeio**
- Build ONE keyword automation per reel: *Automation → Comment → DM (Easy Builder)*
  - Trigger: **any post or reel** + keyword
  - Keyword: the reel's CTA word (e.g. **MCPs** — plural only, exact)
  - Opening DM (branded): "Hey! 🙌 So glad you're here. Tap below and I'll send your free [TOPIC] guide right over 👇" → button
  - Link DM: short line + **the correct guide link** (provided per reel)
- **For volume: don't rebuild — DUPLICATE this automation and swap two things: the keyword + the link.** ~30 sec each.
- Always finish by clicking **Go Live** only after you approve.

**Buffer (scheduling)**
- Needs the finished **MP4**. Upload → caption (from `CAPTION_hooks_hashtags.md`) → schedule to IG/TikTok/YT Shorts.

> Honest scaling note: at a few videos a day, browser-driven is fine. At high volume, ElevenLabs + ManyChat + Buffer all have APIs that would make this fully unattended — that's the future upgrade if the cadence demands it.
